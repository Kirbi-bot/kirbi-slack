...
function checkMessageForCommand (msg, isEdit) {
	function isMention (args) {
		return args[0] === `<@${Kirbi.Slack._self.id}>`;
	};

	//drop our own messages to prevent feedback loops
	if (msg.user === Kirbi.Slack._self.id) {
		return;
	}
	...
	if (isMention(msg.text.split(' '))) {
		if (Kirbi.Config.elizaEnabled) {
			...
			var message = msg.text.replace(`<@${Kirbi.Slack._self.id}> `, '');
	...
	if (msg.text.startsWith(Kirbi.Config.commandPrefix)) {
		...
		var cmdTxt = msg.text.split(' ')[0].substring(Kirbi.Config.commandPrefix.length).toLowerCase();
		var suffix = msg.text.substring(cmdTxt.length + Kirbi.Config.commandPrefix.length + 1); //add one for the ! and one for the space
		...
		if (msg.type === "dm") {
			...
			if (suffix) {
				...
				Kirbi.Slack.sendMessage(info, msg.channel);
			} else {
				Kirbi.Slack.sendMessage('**Available Commands:**', msg.user);
				var batch = '';
				...
				for (var i in sortedCommands) {
					...
					if (newBatch.length > (1024 - 8)) { //limit message length
						Kirbi.Slack.sendMessage(batch, msg.user);
						batch = info;
					} else {
						batch = newBatch
					}
			...
			Kirbi.Slack.sendMessage(`${cmdTxt} not recognized as a command!`);
		}
	};
};
